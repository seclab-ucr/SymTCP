...
def find_paths(func_name, start_ea, end_ea):
    ...
    # Generate the critical branches
    #print(branches)
    critical_branches = {}
    for br, succs in branches.iteritems():
        ...
        if len(succs) == 1:
            ...
            for succ in succs:
                s += "0x%x" % succ
                critical_branches[br] = succ
            print(s)

    print("[End] %s from %x to %x: %d\n" % (func_name, start_ea, end_ea, path_cnt[0]))

    return all_paths, path_cnt[0], critical_branches


found = {}

all_critical_branches = {}

for cc in all_call_chains:
    print("---Call chain starts---\n")
    for func_name, start_ea, call_site, lvl in cc:
        if (start_ea, call_site) not in found:
            paths, cnt, critical_branches = find_paths(func_name, start_ea, call_site)
            for br, target in critical_branches.iteritems():
                if br not in all_critical_branches:
                    all_critical_branches[br] = target
                elif target != all_critical_branches[br] and all_critical_branches[br] != -1:
                    # both target branches visited, label it with -1
                    all_critical_branches[br] = -1
            found[(start_ea, call_site)] = paths
    print("---Call chain ends---\n")

print("------------Critical Branches-------------")
for br, target in all_critical_branches.iteritems():
    if target != -1:
        print("0x%x:0x%x" % (br, target))
